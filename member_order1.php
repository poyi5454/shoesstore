	  ...
	  <table width="100%" height="850" border="0" cellpadding="0" cellspacing="0">
        <tr>
          <td align="left" valign="top"><div class="title">
              <h2><img src="images/diagram-60.png" width="48" height="48" /> ข้อมูลการสั่งซื้อสินค้า </h2>
          </div>
...
$q="SELECT * FROM ".$order."  WHERE(ord_id LIKE '%".$Search."%' AND ord_idmb='".$_SESSION['sess_mb_id']."' AND ord_status IN('Y', '1', '2', '3', '4', '5', '6', 'N') ) ORDER BY ord_status DESC";  
		...
		?>
                      <table width="100%" border="0" cellpadding="0" cellspacing="0">
                        <tr>
                          ...
                          <td width="28%" height="30" align="left" valign="middle" bgcolor="#999999" class="sell"><strong style="padding-left: 5px;">รายชื่อผู้สั่งซื้อ</strong></td>
                          <td width="14%" height="30" align="center" valign="middle" bgcolor="#999999" class="sell"><strong>ราคา</strong></td>
                          <td width="15%" height="30" align="center" valign="middle" bgcolor="#999999" class="sell"><strong>สถานะ</strong></td>
                          <td width="14%" height="30" align="center" valign="middle" bgcolor="#999999" class="sell"><strong>ข้อมูล</strong></td>
                          <td width="17%" height="30" align="center" valign="middle" bgcolor="#999999" class="sellright"><strong> ยกเลิก</strong></td>
                        </tr>
                        <?
	}							
...
?>
                        <li>
                          <tr>
                            ...
                            <td width="28%" height="20" align="left" valign="middle" bgcolor="#F0F0F0" class="sell1" style="padding-left: 5px;"><?=$Result['ord_name']?></td>
                            <td width="14%" height="20" align="center" valign="middle" bgcolor="#F0F0F0" class="sell1"><?=number_format($Result['ord_total'],2)?></td>
                            <td width="15%" height="20" align="center" valign="middle" bgcolor="#F0F0F0" class="sell1">
							<?PHP
// ถ้าสถานะเท่ากับ Y  คือ  ข้อมูลสั่งซื้อสิค้าใหม่ยังไม่ชำระเงิน
	if($Result['ord_status']=='Y'){
						...
						echo "<samp style='color: #FF6600;'>ส่งสินค้าแล้ว แจ้งเลข EMS ในอีเมล</samp>";
				...
				?></td>
                            <td width="14%" height="20" align="center" valign="middle" bgcolor="#F0F0F0" class="sell1">
							<?php if($Result['ord_status']=='Y'){ ?>
							[<a href="payment_member_order1.php?ID=<?=$Result['ord_id']?>&amp;ord=1#<?=$Result['ord_id']?>">แจ้งชำระเงิน</a>]
							<?php }else{ ?>
							[<a href="print.php?ID=<?=$Result['ord_id']?>&amp;ord=1">ข้อมูล</a>]
							<?php } ?>
                            ...
                            <td width="17%" height="20" align="center" valign="middle" bgcolor="#F0F0F0" class="sellright1">
							[<?
				if($Result['ord_status']=='Y'){
							
							echo "<a href='cancle_order.php?ID=".$Result['ord_id']."' onclick=\" return confirm('ต้องการยกเลิกใบสั่งซื้อ $code ')\">ยกเลิก</a>";
							...
							}else if($Result['ord_status']=='3'){
							echo "<a href='#' style=\"color:#666;\" onclick=\" return confirm('ยกเลิกสินค้าไม่ได้ สินค้าอนุมัติแล้วรอบรับสินค้าได้เลยค่ะ')\">ยกเลิก</a>";
							
							
							}else{
							echo "<a href='#' style=\"color:#666;\" onclick=\" return confirm('ยกเลิกสินค้าไม่ได้ อยู่ในระหว่างกำลังดำเนินการ')\">ยกเลิก</a>";
							}
			?>]                            </td>
                          </tr>
                        </li>
