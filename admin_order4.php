	  ...
	  <table width="100%" height="850" border="0" cellpadding="0" cellspacing="0">
        <tr>
          <td align="left" valign="top"><div class="title">
            <h2><img src="images/diagram-60.png" width="48" height="48" /> ข้อมูลใบสั่งซื้อ รอจัดส่งสินค้า </h2>
          </div>
...
$q="SELECT * FROM ".$order."  WHERE(ord_id LIKE '%".$Search."%'  AND ord_status IN('3')) ORDER BY ord_status DESC";  
...
// ถ้าสถานะเท่ากับ Y  คือ  ข้อมูลสั่งซื้อสิค้าใหม่ยังไม่ชำระเงิน
	if($Result['ord_status']=='Y'){
				...
				?></td>
                            <td width="12%" height="20" align="center" valign="middle" bgcolor="#F0F0F0" class="sell1"> [<a href="print.php?m_page=<?=$_GET['m_page']?>&amp;ID=<?=$Result['ord_id']?>&amp;ord=44">ข้อมูล</a>] </td>
